...
// прямоугольник фона на весь экран и панель с рамкой.
func (g *Game) drawBattleOverlayPanel(screen *ebiten.Image) {
...
// заголовок, HP, фазу, подсказки и последний лог.
func (g *Game) drawBattleOverlayText(screen *ebiten.Image) {
	panelX := float32(120)
	panelY := float32(140)
	...
	text.Draw(screen, title, g.hudFace, titleOp)
	...
	text.Draw(screen, fmt.Sprintf("Player HP: %d", g.battle.PlayerHP), g.hudFace, bodyOp1)
	...
	text.Draw(screen, fmt.Sprintf("Enemy HP: %d", g.battle.EnemyHP), g.hudFace, bodyOp2)
	...
	text.Draw(screen, phaseText, g.hudFace, bodyOp3)
	...
	text.Draw(screen, "Space - attack", g.hudFace, bodyOp4)
	...
	text.Draw(screen, "Esc - retreat", g.hudFace, bodyOp5)
	...
	text.Draw(screen, g.battle.LastLog, g.hudFace, bodyOp6)
}

// drawHUDText рисует текстовые блоки HUD (счётчик собранных предметов и т.п.).
func (g *Game) drawHUDText(screen *ebiten.Image) {
	op := &text.DrawOptions{}
	op.GeoM.Translate(10, 20)
	op.ColorScale.ScaleWithColor(color.White)
	text.Draw(screen, fmt.Sprintf("Pickups: %d", g.pickupCount), g.hudFace, op)
}
